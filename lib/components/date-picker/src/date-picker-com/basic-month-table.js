...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { expose, emit }) {
    ...
    const getCellStyle = (cell) => {
      ...
      style.disabled = props.disabledDate ? utils.datesInMonth(props.date, year, month, lang.value).every(props.disabledDate) : false;
      style.current = arrays.castArray(props.parsedValue).some((date) => dayjs__default["default"].isDayjs(date) && date.year() === year && date.month() === month);
    ...
    const isSelectedCell = (cell) => {
      const year = props.date.year();
      const month = cell.text;
      return arrays.castArray(props.date).some((date) => date.year() === year && date.month() === month);
    };
